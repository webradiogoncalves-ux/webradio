...
public class MainActivity extends ComponentActivity {
    ...
    private void loadPlaylist(){ Executors.newSingleThreadExecutor().execute(()->{ try{ URL u=new URL(M3U_URL); HttpURLConnection c=(HttpURLConnection)u.openConnection(); c.setConnectTimeout(15000); c.setReadTimeout(30000); c.setRequestProperty("User-Agent","WebTVFull/1.0"); InputStream in=c.getInputStream(); BufferedReader r=new BufferedReader(new InputStreamReader(in)); String line; Item cur=null; while((line=r.readLine())!=null){ line=line.trim(); if(line.startsWith("#EXTINF:")){ cur=parseExtinf(line); } else if(cur!=null && !line.startsWith("#") && line.length()>5){ cur.url=line; items.add(cur); cur=null; } } r.close(); loadEpg(); runOnUiThread(()->setupCategories()); }catch(Exception e){runOnUiThread(()->status.setText("Erro ao carregar M3U: "+e.getMessage()));}}); }
    private Item parseExtinf(String l){ Item x=new Item(); int comma=l.indexOf(','); x.name=comma>=0?l.substring(comma+1).trim():"Canal"; x.id=attr(l,"tvg-id"); x.group=attr(l,"group-title"); if(x.group.isEmpty())x.group="Outros"; return x; }
    ...
    private void renderList(){ if(list==null)return; list.removeAllViews(); int n=0; for(Item x:items){ if(!currentGroup.equals("Todos")&&!x.group.equals(currentGroup))continue; Button b=new Button(this); b.setText(x.name); b.setTextSize(15); b.setAllCaps(false); b.setGravity(Gravity.START|Gravity.CENTER_VERTICAL); b.setPadding(dp(16),0,dp(12),0); b.setOnClickListener(v->play(x)); list.addView(b,new LinearLayout.LayoutParams(-1,dp(54))); n++; } if(n==0){TextView t=text("Nenhum canal nesta categoria.",14);list.addView(t);} }
    ...
    static class Item{String name="",url="",group="",id="";}
}
