    ...
    private void playPlayback() {
        ...
        try {
            ...
            player.setOnErrorListener((mp, what, extra) -> {
                ...
                connecting = false;
                releasePlayer();
                if (!pausedByUser && retryCount < MAX_RETRIES) {
                    retryCount++;
                    sendState(STATE_ERROR);
                    handler.postDelayed(() -> {
                        if (!pausedByUser) playPlayback();
                    }, 3000L);
                } else {
                    updateNotification();
                    sendState(STATE_ERROR);
            ...
            headers.put("User-Agent", "VemComigoRadio/1.0 Android");
            headers.put("Icy-MetaData", "1");
            player.setDataSource(this, Uri.parse(RadioConfig.RADIO_STREAM_URL), headers);
            player.prepareAsync();
