...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver radioReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            if (!RadioPlaybackService.ACTION_STATE.equals(intent.getAction())) return;
            String state = intent.getStringExtra(RadioPlaybackService.EXTRA_STATE);
            if ("playing".equals(state)) {
                status.setText("Rádio tocando ao vivo");
                ...
                playButton.setText("OUVIR RÁDIO");
            } else if ("volume".equals(state)) {
                status.setText("Volume: " + intent.getIntExtra("volume_percent", 0) + "%");
            }
        }
    };
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 33) registerReceiver(radioReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
    ...
    private void toggleRadio(){
        if(RadioPlaybackService.isPlaying() || RadioPlaybackService.isConnecting()){
            sendRadioAction(RadioPlaybackService.ACTION_PAUSE);
        } else {
            sendRadioAction(RadioPlaybackService.ACTION_PLAY);
        }
    }
    ...
    @Override protected void onDestroy(){
        try { unregisterReceiver(radioReceiver); } catch (Exception ignored) {}
        super.onDestroy();
    }
