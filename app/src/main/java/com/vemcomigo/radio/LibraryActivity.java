...
public class LibraryActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 30 && !Environment.isExternalStorageManager()) {
            try {
                android.content.Intent permission = new android.content.Intent(
                        android.provider.Settings.ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION);
                permission.setData(android.net.Uri.parse("package:" + getPackageName()));
                startActivity(permission);
            } catch (Exception e) {
                Toast.makeText(this, "Permita o acesso aos arquivos nas configurações do aplicativo.", Toast.LENGTH_LONG).show();
            }
