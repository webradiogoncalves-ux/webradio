...
public class AutomationService extends Service {
    // Limite rígido do plano Caster: toda mídia transmitida deve ser <= 96 Kbps.
    private static final int MAX_BITRATE_BPS = 96000;
    ...
    private void streamUri(Uri uri, String title, String programName) throws Exception {
        ...
        try {
            ...
            MediaFormat selected = ex.getTrackFormat(track);
            int br = selected.containsKey(MediaFormat.KEY_BIT_RATE) ? selected.getInteger(MediaFormat.KEY_BIT_RATE) : -1;
            if (br <= 0 || br > MAX_BITRATE_BPS) {
                throw new IOException("Áudio acima do limite de 96 Kbps: " + title);
            }
            ex.selectTrack(track);
            updateNotification("● NO AR • " + title + " • 96 Kbps");
    ...
    private void streamAsset(String assetPath, String label) throws Exception {
        ...
        if (br <= 0 || br > MAX_BITRATE_BPS) {
            ex.release();
            throw new IOException("Áudio interno acima de 96 Kbps: " + assetPath);
        }
        ex.selectTrack(track);
        updateNotification("● NO AR • " + label + " • 96 Kbps");
    ...
    static class IcecastMp3Streamer {
        ...
        void connect() throws Exception {
            ...
            String req = "SOURCE " + mount + " HTTP/1.0\r\n" +
                    ...
                    "Ice-Public: 1\r\n" +
                    "Ice-Bitrate: 96\r\n" +
                    "User-Agent: VemComigoAndroid/2.2-96K\r\n\r\n";
