...
public class MainActivity extends Activity {
 ...
 @Override public void onCreate(Bundle b){super.onCreate(b);setContentView(R.layout.activity_main);bind(); if(Build.VERSION.SDK_INT>=33) requestPermissions(new String[]{"android.permission.POST_NOTIFICATIONS"},90); tick(); refresh();}
 void bind(){status=f(R.id.status);listeners=f(R.id.listeners);bitrate=f(R.id.bitrate);now=f(R.id.now);clock=f(R.id.clock);log=f(R.id.log);
  f(R.id.start).setOnClickListener(v->startLocalAutomation());f(R.id.stop).setOnClickListener(v->stopLocalAutomation());f(R.id.automation).setOnClickListener(v->startLocalAutomation());f(R.id.reconnect).setOnClickListener(v->refresh());
 ...
 void startLocalAutomation(){ final EditText e=new EditText(this); e.setHint("Senha do transmissor Caster"); e.setInputType(129); String old=getSharedPreferences("radio",0).getString("source_password",""); if(!old.isEmpty()) e.setText(old); new AlertDialog.Builder(this).setTitle("INICIAR AUTOMAÇÃO").setMessage("O celular reserva vai tocar a grade e enviar o sinal ao Caster.").setView(e).setNegativeButton("CANCELAR",null).setPositiveButton("INICIAR",(d,w)->{ Intent i=new Intent(this,AutomationService.class); i.putExtra("password",e.getText().toString()); if(Build.VERSION.SDK_INT>=26) startForegroundService(i); else startService(i); getSharedPreferences("radio",0).edit().putBoolean("auto_start",true).apply(); status.setText("● AUTOMAÇÃO LIGADA"); status.setTextColor(Color.rgb(34,197,94)); log.setText("Celular reserva executando a programação."); }).show(); }
 void stopLocalAutomation(){ getSharedPreferences("radio",0).edit().putBoolean("auto_start",false).apply(); stopService(new Intent(this,AutomationService.class)); status.setText("● AUTOMAÇÃO PARADA"); status.setTextColor(Color.rgb(239,68,68)); log.setText("Automação do celular parada."); }
